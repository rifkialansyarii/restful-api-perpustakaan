...
var editable = renderer["a" /* default */].create('<div class="note-editable card-body" contentEditable="true" role="textbox" aria-multiline="true"/>');
...
var ui_checkbox = renderer["a" /* default */].create('<div class="custom-control"></div>', function ($node, options) {
  $node.html(['<label class="custom-control-label"' + (options.id ? ' for="note-' + options.id + '"' : '') + '>', '<input type="checkbox" class="custom-control-input"' + (options.id ? ' id="note-' + options.id + '"' : ''), options.checked ? ' checked' : '', ' aria-label="' + (options.text ? options.text : '') + '"', ' aria-checked="' + (options.checked ? 'true' : 'false') + '"/>', ' ' + (options.text ? options.text : '') + '</label>'].join(''));
});
