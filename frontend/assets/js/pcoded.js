...
function collapsehover() {
    if ($(".pcoded-navbar").hasClass('navbar-collapsed') == true) {
        var $trg = $(".pcoded-navbar.navbar-collapsed");
        smmenu($trg);
    }
}
function smmenu($trg) {
    $trg.off('click').off('mouseenter mouseleave').hover(
        function() {
            $(".pcoded-navbar.navbar-collapsed:not(.theme-horizontal)").find('li.pcoded-trigger').children('.pcoded-submenu').slideDown();
        },
        function() {
            $(".pcoded-navbar.navbar-collapsed:not(.theme-horizontal)").find('li.pcoded-trigger').children('.pcoded-submenu').slideUp();
        }
    );
}
// ===============
...
if (vw < 992) {
    if ($('.page-wrapper>.page-header')[0]) {
        $('.pcoded-header > .collapse:not(.show)>.me-auto').css('display', "none");
        $('.pcoded-header > .collapse:not(.show)>.ms-auto').css('display', "block");
        $('.pcoded-header > .container> .collapse:not(.show)>.me-auto').css('display', "none");
        $('.pcoded-header > .container> .collapse:not(.show)>.ms-auto').css('display', "block");
